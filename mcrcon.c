...
unsigned int mcrcon_parse_seconds(char *str)
{
	...
	if (errno != 0) {
		fprintf(stderr, "-w 无效的值。\n错误 %d: %s\n", errno, strerror(errno));
		exit(EXIT_FAILURE);
	}

	if (end == str) {
		fprintf(stderr, "-w 无效的值。 (不是一个数字?)\n");
		exit(EXIT_FAILURE);
	}

	if (result <= 0 || result > MAX_WAIT_TIME) {
		fprintf(stderr, "-w 值超出范围。\n合法的值在 1 - %d (秒)。\n", MAX_WAIT_TIME);
		exit(EXIT_FAILURE);
	}
	...
	while ((opt = getopt(argc, argv, "vrtcshw:H:p:P:")) != -1)
	{
		switch (opt) {
			...
			case 'v':
				...
				puts("汉化: XiaoMC - https://github.com/lxyminecraft/mcrcon")
			...
			case '?':
			default:
				puts("试试 'mcrcon -h' 或者 'man mcrcon' 获取帮助！");
				exit(EXIT_FAILURE);
		}
	}

	if (pass == NULL) {
		puts("你必须提供控制密码 (-p password)。\n试试 'mcrcon -h' 或者 'man mcrcon' 获取帮助！");
		return 0;
	}
...
{
	puts(
		"汉化：XiaoMC  https://github.com/lxyminecraft/mcrcon\n\n用法: "IN_NAME" [选项] [命令]\n\n"
		"发送 RCON 命令 到 Minecraft 服务器。\n\n"
		"选项:\n"
		...
		"  -c\t\t禁用颜色输出\n"
		"  -r\t\t输出原始数据包\n"
		"  -w\t\t在每个命令之间等待指定的持续时间（单位：秒） (范围：1 - 600)\n"
		"  -h\t\t打印用法\n"
		"  -v\t\t版本信息\n\n"
...
rc_packet *net_recv_packet(int sd)
{
	...
	if (ret == 0) {
		fprintf(stderr, "连接已丢失！\n");
		global_connection_alive = 0;
		return NULL;
	}

	if (ret != sizeof(int)) {
		fprintf(stderr, "错误: recv() 失败. 无效的数据包大小 (%d)。\n", ret);
		global_connection_alive = 0;
		return NULL;
	}
	...
	if (psize < 10 || psize > DATA_BUFFSIZE) {
		fprintf(stderr, "警告: 无效的数据包大小 (%d)。 必须大于 10 且小于 %d。\n", psize, DATA_BUFFSIZE);
...
rc_packet *packet_build(int id, int cmd, char *s1)
{
	...
	if (len >= DATA_BUFFSIZE) {
		fprintf(stderr, "警告: 命令字符串太长 (%d)。允许的最大值: %d。\n", len, DATA_BUFFSIZE - 1);
		return NULL;
	}
...
int run_terminal_mode(int sock)
{
	...
	puts("已登入。\n键入 'RCQUIT' 或者 按下 Ctrl-D / Ctrl-C 来断开连接。");
	...
	while (global_connection_alive) {
		...
		if (strcasecmp(command, "RCQUIT") == 0)
			break;
